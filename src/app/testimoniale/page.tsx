      ...
      <main className="bg-[#ececec] pb-24 pt-10 md:pt-16 lg:pt-[96px]">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {trustPoints.map((item) => (
              <article key={item} className="rounded-[20px] border border-black/5 bg-white p-6">
                <p className="text-[21px] font-medium text-white">Punct de încredere</p>
                <h2 className="mt-3 text-[21px] font-semibold leading-[1.18] tracking-[-0.02em] text-white">{item}</h2>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto mt-8 w-full max-w-[1680px] px-4 md:px-8 lg:mt-10 lg:px-12">
          <div className="rounded-[24px] border border-black/5 bg-white p-6 md:p-10 lg:p-12">
            ...
            <p className="mt-4 max-w-[980px] text-[21px] leading-[1.7] text-[#3f3f3f]">
              ...
              {reviews.map((review) => (
                <article key={review.name + review.meta} className="flex min-h-[230px] flex-col rounded-[18px] border border-black/5 bg-[#f7f7f7] p-6 transition duration-300 hover:-translate-y-[2px]">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[21px] font-semibold text-white">{review.name}</h3>
                    <span className="text-[21px] font-semibold text-white">★★★★★</span>
                  </div>
                  <p className="mt-1 text-[21px] text-white">{review.meta}</p>
                  <p className="mt-4 text-[21px] leading-[1.65] text-[#3f3f3f]">{review.text}</p>
                </article>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[24px] border border-black/5 bg-white p-6 md:p-10 lg:p-12">
            ...
            <div className="mt-7 grid grid-cols-1 gap-4 md:grid-cols-2">
              <article className="rounded-[18px] border border-black/5 bg-[#f7f7f7] p-6">
                <h3 className="text-[21px] font-semibold text-white">Înainte de tratament</h3>
