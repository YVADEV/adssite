    ...
    <PrototypeFrame>
      <main className="bg-black pb-24 pt-12 text-white md:pt-16 lg:pt-[110px]">
        <section className="mx-auto w-full max-w-[1680px] px-4 md:px-8 lg:px-12">
          <p className="text-[12px] font-medium uppercase tracking-[0.14em] text-white/70">Transparență totală</p>
          <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2 text-[12px] text-white">
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:gap-8">
                ...
                {pricingData.map((category) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[18px] leading-none ${isActive ? "text-[#8fd17f]" : "text-white/50"}`}>+</span>
                    </motion.button>
                  );
                })}
              </div>
            ...
            <article className="rounded-[24px] border border-white/10 bg-white/[0.04] p-5 md:p-6 lg:p-7">
              <div className="border-b border-white/10 pb-5">
                ...
                <p className="mt-2 text-[14px] text-white/65">{activeData.items.length} proceduri disponibile</p>
              </div>
              ...
              <div className="mt-5 max-h-[620px] space-y-0 overflow-y-auto pr-2">
                {activeData.items.map((entry) => (
                  <div key={`${activeData.category}-${entry.name}`} className="group flex items-end gap-3 border-b border-white/10 py-[10px]">
                    <p className="text-[14px] leading-[1.45] text-white">{entry.name}</p>
                    <span className="mb-[6px] flex-1 border-b border-dotted border-white/25" />
                    <p className="shrink-0 text-right text-[14px] font-semibold text-white">{entry.price}</p>
                  </div>
                ))}
              </div>

              <div className="mt-6 rounded-[16px] border border-[rgba(79,127,71,0.35)] bg-[rgba(79,127,71,0.08)] p-4">
                <p className="text-[18px] font-semibold text-white">Solicită un plan personalizat</p>
                <p className="mt-1 text-[13px] text-white/75">Te contactăm rapid pentru o estimare clară și pașii potriviți cazului tău.</p>
                <a href="/#contact" className="mt-3 inline-flex h-[42px] items-center rounded-full bg-white px-5 text-[14px] font-semibold text-black transition duration-300 hover:scale-[1.02]">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="rounded-[16px] border border-white/10 bg-white/[0.04] p-4">
              <p className="text-[12px] uppercase tracking-[0.1em] text-white/60">Consultație</p>
              <p className="mt-2 text-[24px] font-semibold leading-none text-white">de la 150 RON</p>
            </div>
            <div className="rounded-[16px] border border-white/10 bg-white/[0.04] p-4">
              <p className="text-[12px] uppercase tracking-[0.1em] text-white/60">Implantologie</p>
              <p className="mt-2 text-[24px] font-semibold leading-none text-white">de la 400 EURO</p>
            </div>
            <div className="rounded-[16px] border border-white/10 bg-white/[0.04] p-4">
              <p className="text-[12px] uppercase tracking-[0.1em] text-white/60">Alignere</p>
