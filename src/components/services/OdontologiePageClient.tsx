    ...
    <main className="bg-[#ececec] pb-24 pt-14 md:pt-20 lg:pt-[130px]">
      <section className="mx-auto w-full max-w-[1680px] px-4 md:px-8 lg:px-12">
        <motion.div {...reveal} className="max-w-[1200px]">
          <p className="text-[12px] font-medium uppercase tracking-[0.13em] text-[#5c5c5c]">Stomatologie restaurativă</p>
          ...
          <p className="mt-7 max-w-[1040px] text-[16px] leading-[1.75] text-[#373737] md:text-[17px]">
            La Alverna Dental Studio Cluj oferim tratamente de odontologie la standarde înalte, cu tehnologii moderne
            și materiale biocompatibile. Intervenim prompt pentru tratarea leziunilor carioase și necarioase, astfel
            încât să prevenim complicațiile și să redăm funcționalitatea și estetica naturală a dintelui.
          </p>
      ...
      <section className="mx-auto mt-7 w-full max-w-[1680px] px-4 md:px-8 lg:px-12">
        <motion.div {...reveal} className="relative overflow-hidden rounded-[28px]">
          <img src="/spark-hero.png" alt="Odontologie Cluj - Alverna Dental Studio" className="h-[56vw] max-h-[700px] min-h-[320px] w-full object-cover saturate-[0.84] brightness-[0.84] transition duration-700 hover:scale-[1.02]" />
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/44 via-black/18 to-black/10" />
          <p className="absolute left-5 top-5 rounded-full border border-white/25 bg-black/30 px-4 py-1.5 text-[11px] font-medium uppercase tracking-[0.12em] text-white">Prevenție și tratament minim-invaziv</p>
        </motion.div>
      </section>
      ...
      <section className="mx-auto mt-16 w-full max-w-[1680px] px-4 md:px-8 lg:px-12">
        <motion.article {...reveal} className="grid border-t border-black/12 pt-10 lg:grid-cols-[minmax(260px,0.95fr)_minmax(0,1.55fr)] lg:gap-14">
          <h2 className="text-[32px] font-semibold leading-[1.03] tracking-[-0.03em] text-[#0A0A0A] md:text-[44px]">Tratamente de odontologie în Cluj</h2>
          <p className="mt-5 text-[16px] leading-[1.75] text-[#3a3a3a] lg:mt-0">
            Odontologia este un pilon central al stomatologiei moderne și urmărește identificarea, tratarea și
            prevenirea afecțiunilor care compromit structura dentară. Intervenția la timp reduce riscul infecțiilor
            pulpare, previne pierderea dintelui și evită tratamente ulterioare mai invazive.
          </p>
        </motion.article>
        ...
        <motion.article {...reveal} className="mt-14 grid border-t border-black/12 pt-10 lg:grid-cols-[minmax(260px,0.95fr)_minmax(0,1.55fr)] lg:gap-14">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Proceduri frecvente</h3>
        ...
        <motion.article {...reveal} className="mt-14 grid border-t border-black/12 pt-10 lg:grid-cols-[minmax(260px,0.95fr)_minmax(0,1.55fr)] lg:gap-14">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Când și de ce apar leziunile dentare</h3>
          <ul className="mt-5 space-y-3 text-[16px] leading-[1.75] text-[#3b3b3b] lg:mt-0">
            {lesionCauses.map((item) => (
              <li key={item} className="flex gap-3">
                <span className="mt-[10px] h-[4px] w-[4px] rounded-full bg-[#4E7044]" />
                <span>{item}</span>
              </li>
        ...
        <motion.article {...reveal} className="mt-14 grid border-t border-black/12 pt-10 lg:grid-cols-[minmax(260px,0.95fr)_minmax(0,1.55fr)] lg:gap-14">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Simptomele cariilor dentare</h3>
          <ul className="mt-5 space-y-3 text-[16px] leading-[1.75] text-[#3b3b3b] lg:mt-0">
            {cariesSymptoms.map((item) => (
              <li key={item} className="flex gap-3">
                <span className="mt-[10px] h-[4px] w-[4px] rounded-full bg-[#4E7044]" />
                <span>{item}</span>
              </li>
        ...
        <motion.article {...reveal} className="mt-14 border-t border-black/12 pt-10">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Etapele tratamentului odontologic</h3>
        ...
        <motion.article {...reveal} className="mt-14 grid border-t border-black/12 pt-10 lg:grid-cols-[minmax(260px,0.95fr)_minmax(0,1.55fr)] lg:gap-14">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Avantajele tratamentului în clinică specializată</h3>
        ...
        <motion.article {...reveal} className="mt-14 border-t border-black/12 pt-10">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Odontologia medico-legală</h3>
          <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {forensicBlocks.map((item, idx) => (
              <motion.article key={item.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.55, delay: idx * 0.08 }} className="border-t border-black/12 pt-4">
                <h4 className="text-[18px] font-semibold text-[#0A0A0A]">{item.title}</h4>
                <p className="mt-2 text-[15px] leading-[1.65] text-[#3b3b3b]">{item.text}</p>
              </motion.article>
        ...
        <motion.article {...reveal} className="mt-14 border-t border-black/12 pt-10">
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] text-[#0A0A0A] md:text-[40px]">Întrebări frecvente despre odontologie</h3>
          <div className="mt-7 divide-y divide-black/10">
      ...
      <section id="contact" className="relative w-full overflow-hidden bg-[#0A0A0A] py-20 md:py-[120px]">
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-[1680px] grid-cols-1 gap-12 px-4 md:px-8 lg:grid-cols-[430px_1fr] lg:gap-20 lg:px-12">
          ...
          <div className="pt-0 text-white lg:pt-8">
            <h3 className="max-w-[720px] text-[42px] font-semibold leading-[1.04] tracking-[-0.04em]">Tratează la timp, păstrează-ți dinții sănătoși.</h3>
            <p className="mt-5 max-w-[680px] text-[16px] leading-[1.55] text-[#d6d6d6]">
              Echipa Alverna Dental Studio te ajută cu tratamente odontologice rapide, precise și personalizate,
              pentru rezultate durabile și confort real.
            </p>
      ...
      <section className="mx-auto mt-16 w-full max-w-[1680px] px-4 md:px-8 lg:px-12">
        <motion.div {...reveal} className="relative overflow-hidden rounded-[28px] bg-[#0A0A0A] p-8 text-white md:p-10 lg:p-12">
          ...
          <h3 className="text-[30px] font-semibold leading-[1.05] tracking-[-0.028em] md:text-[40px]">Ai nevoie de un consult de odontologie în Cluj?</h3>
          <p className="mt-4 max-w-[980px] text-[16px] leading-[1.7] text-white/82">
            Programează-te la Alverna Dental Studio și primește un plan de tratament adaptat exact nevoilor tale.
          </p>
