      ...
      <main className="bg-[#ececec] pb-24 pt-10 md:pt-16 lg:pt-[96px]">
          ...
          <div className="rounded-[28px] border border-black/5 bg-white px-6 py-10 md:px-10 md:py-14 lg:px-14 lg:py-16">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {trustPoints.map((item) => (
                <span key={item} className="rounded-full border border-black/10 bg-[#f8f8f8] px-4 py-2 text-[13px] font-medium text-white">
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {trustPoints.map((item) => (
              <article key={item} className="rounded-[20px] border border-black/5 bg-white p-6">
                <p className="text-[14px] font-medium text-white">Punct de încredere</p>
              ...
              {reviews.map((review) => (
                <article key={review.name + review.meta} className="flex min-h-[230px] flex-col rounded-[18px] border border-black/5 bg-[#f7f7f7] p-6 transition duration-300 hover:-translate-y-[2px]">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[16px] font-semibold text-white">{review.name}</h3>
                    <span className="text-[14px] font-semibold text-white">★★★★★</span>
                  </div>
                  <p className="mt-1 text-[12px] text-white">{review.meta}</p>
        ...
        <section className="mx-auto mt-8 w-full max-w-[1680px] px-4 md:px-8 lg:mt-10 lg:px-12">
          <div className="rounded-[24px] bg-[#0A0A0A] p-6 text-white md:p-10 lg:p-12">
            ...
            <p className="mt-4 max-w-[900px] text-[15px] leading-[1.7] text-white">
        ...
        <section className="mx-auto mt-8 w-full max-w-[1680px] px-4 md:px-8 lg:mt-10 lg:px-12">
          <div className="rounded-[24px] bg-[#4E7044] p-8 text-white md:p-12">
            <p className="text-[12px] font-medium uppercase tracking-[0.14em] text-white">Programări</p>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <a href="/#contact" className="inline-flex h-[44px] items-center rounded-full bg-white px-6 text-[14px] font-semibold text-white transition duration-300 hover:scale-[1.02]">
                Solicită evaluare
              </a>
